...
func update_cursor(camera3d : Camera3D, current_unit : BaseUnit):
	...
	if result:
		...
		self.direction_to_cursor = (self.position - current_unit.position).normalized()
		self.distance_to_cursor = (self.position - current_unit.position).length()
